<template>
  <div class="fixed pt-0 md:pt-3 lg:pt-10 ">
    ...
    >
      <div
        class="bg-blue-200 dark:bg-blue-800 px-10 py-3 text-xl font-mono tracking-wider rounded-md flex flex-col md:flex-row justify-between items-center "
      >
        <div>{{ item.brand }}:{{ item.name }}</div>
        <div class="flex mt-4 md: md:m-0 text-sm">
          <div
            class="flex items-center bg-green-600 p-1 mx-3 px-3 cursor-pointer rounded-md text-white" @click="editItem"
          >
            <span class="material-icons">edit</span>Edit
          </div>
          <div
            class="flex items-center bg-red-600 p-1 mx-3 px-3 cursor-pointer rounded-md text-white" @click="deleteItem"
          >
            <span class="material-icons">delete</span>Delete
          </div>
          <span
            class="material-icons p-2 bg-white text-black rounded-full ml-10 -mr-5 cursor-pointer "
            @click="close"
            >close</span
          >
        </div>
